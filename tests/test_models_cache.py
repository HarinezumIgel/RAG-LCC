...
class FakePooling:
    def __init__(self, word_embedding_dimension=None, pooling_mode_mean_tokens=None):
...
class TestSwitch2Device:
    def test_cpu_when_use_cpu_flag_set(self):
        ...
        device, device_type, dtype, idx = mc.switch2device()
...
class TestLoadQuantizedModel:
    ...
    def test_cache_key_includes_dtype(self, monkeypatch):
        ...
        model1 = mc.load_quantized_model("same-model")
...
class TestGetCrossEncoder:
    def test_returns_cross_encoder(self, monkeypatch):
        ...
        assert encoder.kwargs["model_name_or_path"] == "test-cross-model"

    def test_passes_chunk_size_as_max_length(self, monkeypatch):
    ...
    def test_hfdownloader_fallback_on_local_failure(self, monkeypatch):
        """When local load fails, HFDownloader is called and local is retried."""
        mc = _build_models_cache()

        call_count = 0

        def cross_encoder_stub(**kwargs):
            nonlocal call_count
            call_count += 1
            if call_count == 1:
        ...
        monkeypatch.setattr(_mc_mod, "CrossEncoder", cross_encoder_stub)
        ...
        assert len(download_calls) == 1
        assert download_calls[0] == "_MODELS._CROSS"
