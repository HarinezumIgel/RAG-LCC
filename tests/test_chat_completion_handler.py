...
class TestChatCompletionRequest:
    ...
    def test_rag_specific_fields_accepted(self):
        ...
        assert req.chroma_k_value == 30
        assert req.rerank is False
        assert req.chroma_threshold == 0.6
        assert req.chunks_window == 20
        assert req.chroma_weight == 0.5
    ...
    def test_string_int_coerced(self):
        ...
        assert req.chroma_k_value == 200
        assert req.chunks_window == 50
    ...
    def test_float_to_int_coerced(self):
        ...
        req = ChatCompletionRequest(
            model="m",
            messages=[{"role": "user", "content": "q"}],
            chroma_k_value=100.0,
            max_tokens=400.0,
            chunks_window=50.0,
        )
        assert req.chroma_k_value == 100
        assert isinstance(req.chroma_k_value, int)
        assert req.max_tokens == 400
        assert isinstance(req.max_tokens, int)
        assert req.chunks_window == 50
        assert isinstance(req.chunks_window, int)
...
class TestApplyRequestToSession:
    ...
    def test_valid_strategy_applied(self):
        for strat in ("ULTRA_WIDE", "WIDE", "MEDIUM", "NARROW"):
            session = StubSession()
    ...
    def test_chroma_k_value_applied(self):
        req = _base_req(chroma_k_value=42)
        session = self._apply(req)
        assert session.chroma_k_value == 42

    def test_chroma_k_value_not_applied_when_none(self):
        session = StubSession()
        session.chroma_k_value = 99
        self._apply(_base_req(), session=session)
        assert session.chroma_k_value == 99  # unchanged
    ...
    def test_chroma_weight_clamped(self):
        assert self._apply(_base_req(chroma_weight=-1.0)).chroma_weight == 0.0
        assert self._apply(_base_req(chroma_weight=2.0)).chroma_weight == 1.0
        assert abs(self._apply(_base_req(chroma_weight=0.5)).chroma_weight - 0.5) < 1e-9

    def test_chunks_window_applied(self):
        session = self._apply(_base_req(chunks_window=15))
        assert session.chunks_window == 15
    ...
    def test_base_kwargs_set(self):
        req = _base_req(chroma_k_value=20)
        session = self._apply(req)
...
class TestHandleRequest:
    ...
    def test_collection_not_found_raises_404(self):
        ...
        req = _base_req(stream=False)
        with pytest.raises(HTTPException) as exc_info:
            self._run_handle(req, rag=MissingRAG())
        assert exc_info.value.status_code == 404

    # --- LLMResultError → 502 -----------------------------------------------

    def test_llm_result_error_raises_502(self):
        ...
        req = _base_req(stream=False)
        with pytest.raises(HTTPException) as exc_info:
            self._run_handle(req, chatter=FailingChatter())
    ...
    def test_unexpected_exception_raises_500(self):
        ...
        req = _base_req(stream=False)
        with pytest.raises(HTTPException) as exc_info:
            self._run_handle(req, rag=BrokenRAG())
