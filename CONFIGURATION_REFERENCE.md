<!-- markdownlint-disable MD033 MD060 MD024 MD022 MD031 -->
...
This reference covers both a **quick-scan overview** (tables by topic and by config file) and the
**full per-file deep reference** with defaults, code samples, and links to the architecture guide.

---

## 🗂️ Quick Reference: Configurable Components by Topic

### 📄 Document Processing & Chunking

| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **Chunkers** | `RECURSIVE`, `SEMANTIC`, `HEADING`, `SLIDING_WINDOW`, `SENTENCE_WINDOW`, `SLIDE`, `PDF_PAGE` | Split documents into retrievable chunks. Each chunker uses different strategies: RECURSIVE uses fixed word counts with overlap; SEMANTIC breaks at semantic boundaries using embedding similarity; HEADING preserves document structure; SLIDING_WINDOW overlaps sentences; SLIDE/PDF_PAGE are format-specific. |
| **Chunker Profiles** | `DETAILED`, `FAST` | Control chunker routing per file type. DETAILED maps specific chunkers to file extensions (e.g., PDF→PDF_PAGE, .docx→HEADING); FAST uses RECURSIVE for everything. DETAILED provides better quality but slower processing. |
| **Chunk Size** | 200-256 words (configurable per chunker) | Maximum words per chunk. Affects retrieval granularity and context window usage. Larger chunks provide more context but reduce precision. |
...
| **HNSW Parameters** | `THOROUGH` (512 neighbors), `COMPACT` (64 neighbors) | Control ChromaDB vector index structure. THOROUGH explores more neighbors for better recall at cost of speed/memory; COMPACT favors speed and precision. |
| **BM25 Parameters** | `k1`: 1.2, `b`: 0.75, `rrf_k`: 60.0 | Okapi BM25 tuning. k1 controls term-frequency saturation; b controls length normalization; rrf_k is the Reciprocal Rank Fusion constant for combining retrievers. |
...
| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **Web Search** | `WEB_SEARCH_MODE`: "0" (disabled) / "1" (enabled) | Master switch for internet retrieval. When enabled, adds DuckDuckGo search results to retrieval pipeline via RRF fusion. |
| **Web Search Backend** | `duckduckgo` (only implemented) | Search provider. Brave, Tavily, Bing recognized but not implemented. DuckDuckGo requires no API key. |
| **Web Search Filters** | `bm25_pre_filter`: 0.10, `cosine_pre_filter`: 0.30, `rerank_threshold`: 0.50 | Multi-stage filtering of web results. BM25 filters by keyword relevance; cosine by semantic similarity; rerank after cross-encoder scoring. |
| **Web Search Weight** | `web_weight`: 0.5 (default), range 0.0-1.0+ | RRF weight for web results relative to local retrievers (1.0). 0.5 means local results naturally outrank web; 1.0 gives equal weight; >1.0 favors web. |
| **Web Intent Filter** | Entity categories, weight thresholds, `block_on_injection`: True | Blocks harmful web queries before they reach the search API. Intent is scored via entity extraction and verb analysis; categories and refuse/warn thresholds are extensible in `WEB_SEARCH_INTENT_EXTENSIONS`. |

### 🤖 Models & Inference

| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **LLM — Generation** | `mistral:7b`, `llama3.1:8b`, any Ollama/vLLM model | Primary language model for answer generation; role selected with `_ACTIVE_LLM` in `Config_Models.py`. |
| **LLM — Guard** | `llama-guard3:8b` (or any Ollama model) | Separate LLM role for prompt/response safety checking; configured independently via `_ACTIVE_LLM_CHK`. Runs against every user input and generated answer. |
...
| **CPU vs GPU** | `USE_CPU`: True/False, `EMBEDDER_BITS`: 16/32 | Forces CPU-only mode or enables GPU acceleration. Set EMBEDDER_BITS=32 when USE_CPU=True (FP16 requires GPU with accelerate). |
| **License Consent** | `ModelGovernance/licenses/` | Tracks accepted model licenses. Required before model download or first use; updated automatically on consent. |

### 🛡️ Compliance & Content Safety

| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **Banned Word Algorithms** | `Regex`, `Jaccard`, `BM25`, `KeyBERT`, `Levenshtein` | Multi-algorithm detection pipeline. Regex+Levenshtein catches exact and near-matches; Jaccard uses character n-gram overlap; BM25 uses IR scoring; KeyBERT uses semantic similarity. All run in parallel and a consensus count gates the block decision. |
| **Banned Word Thresholds** | Per-algorithm: 0.2-1.0 (normalized scores) | Minimum score per algorithm to count as a hit. Example: Regex=1.0 (exact), Jaccard=0.75 (75% n-gram overlap), KeyBERT=0.45 (semantic similarity). |
...
| **Chat History Storage** | `turns`: 5-10, `prune_batch`: 5 | Maximum stored turns before oldest batch is summarized and discarded. Controls conversational memory budget for long sessions. |
| **Chunk Near-Dedup** | `enabled`: True, `threshold`: 0.85 (Jaccard) | Removes near-duplicate chunks from the retrieval pool after RRF fusion and before reranking. Keeps only the higher-ranked copy when chunks share ≥ threshold word-token fraction. |
| **Answer Grounding** | `min_sentence_tokens`: 5, `min_fragment_len`: 12, `min_overlap_window`: 5 | Controls how strictly answer sentences must overlap with retrieved chunk text to be marked as grounded. Lower values accept shorter fragments; higher values require longer contiguous matches. |
| **Answer Colors** | `highlight`, `answer_mark`, `answer_ansi` in `_MARKED_DOCS_COLORS` | Separate color controls for source-chunk highlighting, grounded-answer Markdown/HTML markup, and CLI ANSI terminal output. |
| **KeyBERT Extraction** | `TOP_N_FIRST`: 60-150, `TOP_N_SECOND`: 30-100, profiles: `STRICT`, `BALANCED`, `RECALL` | Controls keyword/phrase extraction volume. Used for query expansion and classification. STRICT minimizes noise; RECALL maximizes coverage. |
...
| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **Classification Keys** | User-defined list (`_YOUR_CLASSIFICATION_KEYS`) | Fields the LLM must extract during classification. Fully customisable — the prompt templates must be updated to match. Example: "Animal", "Mammal", "Language", "Topic". |
| **Extraction Profiles** | `STRICT`, `BALANCED`, `RECALL` | Pre-configured LLM parameter sets for DocClassify. STRICT (temperature=0, top_k=1) for constrained output; RECALL (temperature=0.1, top_k=40) for broader extraction. |
| **Summary Sentence Count** | `SUMMARY_SENTENCE_CNT`: 10 | Maximum sentences in the "Purpose" summary field generated by DocClassify. |
| **Reverse Stemming** | `REVERSE_STEMMING`: True/False | Converts stemmed keywords back to original surface forms before CSV output. Improves readability of extracted classification fields. |
| **CSV-Filtered Load** | `LOAD_FROM_CLASSIFY_CSV` + `CLASSIFY_CSV_QUERY` (SQLite WHERE) | Load only documents satisfying a SQL query on a prior DocClassify CSV output — the classify-then-selectively-load workflow. |
| **Grounding Markers** | Sentence-level source citations | Answer sentences are marked as grounded only when they overlap with retrieved chunk text. Sensitivity is tunable via `_MARKED_DOCS_GROUNDING`. |

### 🐛 Debugging & Logging

| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **Debug Levels** | `None`: 0, `Basic`: 10, `Service`: 20, `Query Rewrite`: 29, `Standard`: 30, `Prompt Check Input`: 31, `Chunk Content`: 32, `Grounding`: 33, `Algos`: 40, `Components`: 55, `Chat Prompt`: 60, `Extracted Content`: 70, `Ollama Response`: 80, `Streaming`: 100 | Hierarchical debug output verbosity. Higher levels include all lower levels. Standard (30) shows pipeline flow; Chunk Content (32) shows full retrieved text; Chat Prompt (60) shows LLM inputs; Streaming (100) shows raw token output. |
| **Debug Level Format** | `30` (exact), `ge 30` (≥30), `is 33` (==33), `none` (silent) | How to specify debug levels. Can target exact level, minimum level, or specific level only. |
| **Performance Logging** | `PERFORMANCE_LOGGING`: True/False | Logs start/stop timestamps of all major operations to logs/Performance/perf.log. Used for profiling and optimization. |

### ⚙️ System & Environment

| **Component** | **Options/Values** | **Purpose** |
|---------------|-------------------|-------------|
| **Request Timeout** | `REQUEST_TIMEOUT`: 600 seconds | Maximum wait time for Ollama/OpenAI API responses. Prevents hanging on slow inference or network issues. |
...
## 🗂️ Quick Reference: Config File and Slot Name

### 📄 Document Processing & Chunking

| **Component** | **Config File** | **Slot Name / Path** | **Notes** |
|---------------|----------------|----------------------|-----------|
| **Version** | `Config_Global.py` | `_VERSION` | Semantic version string shown at startup |
| **Document Directory** | `Config_Global.py` | `DOC_DIR` | Root folder for documents to index |
| **Chunkers - Recursive** | `Config_Global.py` | `_CHUNKERS["RECURSIVE"]` | CHUNK_SIZE, CHUNK_OVERLAP, PRESERVE_NEWLINES |
| **Chunkers - Semantic** | `Config_Global.py` | `_CHUNKERS["SEMANTIC"]` | MAX_CHUNK_SIZE, BREAKPOINT_PERCENTILE, EMBED_BATCH_SIZE, MIN_SENTENCE_WORDS |
| **Chunkers - Heading** | `Config_Global.py` | `_CHUNKERS["HEADING"]` | MAX_CHUNK_SIZE, PRESERVE_NEWLINES, BREADCRUMB_MODE |
...
| **Collection Name** | `Config_Global.py` | `COLLECTION` | Active ChromaDB collection name |
| **Retrieval Stores Keep** | `Config_Global.py` | `RETRIEVAL_STORES_KEEP` | True=preserve, False=wipe on startup |
| **Exclusions Directory** | `Config_Global.py` | `_EXCLUSIONS_DIR` | Location of exclusion CSV files |
...
| **Component** | **Config File** | **Slot Name / Path** | **Notes** |
|---------------|----------------|----------------------|-----------|
| **Web Search Master Switch** | `Config_Internet_Env.py` | `os.environ["WEB_SEARCH_MODE"]` | "0"=disabled, "1"=enabled |
| **Web Search Backend** | `Config_WebSearch.py` | `_WEB_SEARCH["backend"]` | "duckduckgo" (only implemented) |
| **Web Search Max Results** | `Config_WebSearch.py` | `_WEB_SEARCH["max_results"]` | Fetch limit per query |
| **Web Search Filters** | `Config_WebSearch.py` | `_WEB_SEARCH["bm25_pre_filter"]`, `["cosine_pre_filter"]`, `["rerank_threshold"]` | Multi-stage filtering |
| **Web Search Weight** | `Config_WebSearch.py` | `_WEB_SEARCH["default_web_weight"]` | RRF weight relative to local retrievers |
| **Web Intent Filter Extensions** | `Config_WebSearch.py` | `WEB_SEARCH_INTENT_EXTENSIONS` | Custom entity categories and thresholds |
| **OpenWebUI Web Default** | `Config_WebSearch.py` | `_OPENWEB_UI_WEBSEARCH` | Auto-enable web search for OpenWebUI |

### 🤖 Models & Inference

| **Component** | **Config File** | **Slot Name / Path** | **Notes** |
|---------------|----------------|----------------------|-----------|
| **Active LLM** | `Config_Models.py` | `_ACTIVE_LLM` | Implementation key for main generation model |
| **Active LLM Guard** | `Config_Models.py` | `_ACTIVE_LLM_CHK` | Implementation key for safety check model |
...
| **RAGChatService API Key** | `Config_Models.py` | `_MODELS["ragchatservice"]["_RAGCHATSERVICE"]["API_KEY"]` | Bearer token for OpenWebUI/client authentication |
| **HF API Key** | `Config_Models.py` | `_HF_API_KEY` | Global HuggingFace token; per-model `HF_API_KEY` overrides this |
| **CPU vs GPU** | `Config_Global.py` | `USE_CPU`, `EMBEDDER_BITS` | Force CPU mode; EMBEDDER_BITS=32 required when USE_CPU=True |
...
| **Component** | **Config File** | **Slot Name / Path** | **Notes** |
|---------------|----------------|----------------------|-----------|
| **DocClassify Extraction Config** | `Config_DocClassify.py` | `_ACTIVE_EXTRACTION_CONFIG` | "STRICT", "BALANCED", or "RECALL" |
| **DocClassify LLM Params** | `Config_DocClassify.py` | `_EXTRACTION_MODEL_PARAMS[config]` | Per-config temperature, top_k, top_p |
| **DocClassify KeyBERT** | `Config_DocClassify.py` | `_KEY_BERT[config]` | Per-config TOP_N_FIRST, TOP_N_SECOND, NGRAM ranges |
| **Reverse Stemming** | `Config_DocClassify.py` | `REVERSE_STEMMING` | Convert stems back to surface forms |
| **Classification Keys** | `Config_DocClassify.py` | Custom user list | Fields to extract |

### 🐛 Debugging & Logging

| **Component** | **Config File** | **Slot Name / Path** | **Notes** |
|---------------|----------------|----------------------|-----------|
| **Debug Levels** | `Config_Global.py` | `_ALLOWED_DEBUG_LEVELS` | Dict mapping names to numeric levels |
| **Active Debug Level** | `Config_Global.py` | `DEBUG_LEVEL` | Current debug verbosity (0-100) |
| **URL Debug** | `Config_Global.py` | `URL_DEBUG` | Enable urllib HTTP debugging |
| **HF Debug** | `Config_Global.py` | `HF_DEBUG` | Enable Hugging Face library debugging |
| **Performance Logging** | `Config_Global.py` | `PERFORMANCE_LOGGING` | Enable perf event logging |
| **Log Directory** | `Config_Global.py` | `_LOG_DIRECTORY` | Root logs folder |
| **Network Trace** | `Config_Internet_Env.py` | `os.environ["RAG_LCC_NW_TRACE"]` | Enable socket-level debugging |
| **Stack Trace** | `Config_Internet_Env.py` | `os.environ["RAG_LCC_STACK_TRACE"]` | Enable full error stack traces |
...
### 🤖 Models
**Change the active LLM:**
```python
# Config_Models.py
_ACTIVE_LLM = "mistral"  # or "llama", etc.
```

**Change embedder or cross-encoder:**
```python
# Config_Models.py
...
### 📄 Document Processing
**Change chunking profile:**
```python
# Config_Global.py
_ACTIVE_CHUNKER_CONFIG = "DETAILED"  # or "FAST"
```

**Adjust chunk sizes:**
```python
# Config_Global.py
_CHUNKERS["RECURSIVE"]["CHUNK_SIZE"] = 256
_CHUNKERS["SEMANTIC"]["MAX_CHUNK_SIZE"] = 256
```

**Rebuild collections from scratch:**
```python
# Config_Global.py
RETRIEVAL_STORES_KEEP = False  # Set to True afterward for incremental mode
```
...
os.environ["WEB_SEARCH_MODE"] = "1"  # "0" to disable
```

**Adjust web search weight:**
```python
# Config_WebSearch.py
_WEB_SEARCH["default_web_weight"] = 0.5  # 0.0-1.0+
```
...
```python
# Config_Global.py
DEBUG_LEVEL = 30  # Standard; use "ge 30" for >=30, "is 33" for exact level
```

**In-chat debug adjustment:**
```text
set debug ge 30    # Enable debug level >=30
set debug none     # Disable debugging
```
...
| `rrf_k` | `60.0` | Reciprocal Rank Fusion constant. A large value (e.g. 60) favours agreement between lists over individual rank position; a small value (e.g. 1) favours top-ranked items. 60 is the standard value from the original RRF paper. |
...
The per-session knobs (`web_search`, `web_weight`, `fetch_page_content`) control web retrieval for the active session:

| Switch | Values | Effect |
| --- | --- | --- |
| `web_search` | `'local_only'` / `'local_and_web'` / `'web_only'` | `local_only` — local indexes only (default). `local_and_web` — DuckDuckGo + local retrieval. `web_only` — skip local indexes entirely. Requires `WEB_SEARCH_MODE = "1"`. Session-persistent. |
| `web_weight` | `None` (config default) / float | RRF weight for web results relative to local retrievers (Vector/BM25/Graph = 1.0). Default `0.5` — every local result naturally outranks any web result. Set to `1.0` for equal influence. Pre-set per strategy via `web_weight` in `_STRATEGIES`. |
| `fetch_page_content` | `'snippets only'` / `'fetch pages'` | `'snippets only'` uses only the DuckDuckGo snippet. `'fetch pages'` fetches the full page body via `httpx` — richer context at higher latency. In both modes the original snippet is kept in `metadata["snippet"]` for cross-encoder scoring; the full page (when fetched) goes only to the LLM prompt. Session-persistent. |
...
To hard-switch topics without clearing history or disabling `use_chat_context`, prefix your query with **`new:`** (or `new topic:`):
...
The prefix is stripped before translation and retrieval — the rewriter LLM is skipped for that turn only, so the next turn resumes normal coreference resolution.

See [ARCHITECTURE.md § Query Rewrite](ARCHITECTURE.md#query-rewrite-coreference-resolution) for the full rewrite-flow diagram.
...
- Configuration deep-dive: see this document
