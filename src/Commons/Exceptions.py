...
class BackendUnavailableError(RAGLCCException):
    """Raised when the active LLM backend is unreachable."""

    def __init__(self, message: str, *, provider: str = "backend") -> None:
        super().__init__(message)
        self.provider = provider.lower()
        self.backend_name = self.provider.upper() if self.provider else "BACKEND"


class OllamaNotRunning(BackendUnavailableError):
    """Raised when the Ollama backend is not running or unreachable."""

    def __init__(self, message: str) -> None:
        super().__init__(message, provider="ollama")


class VllmNotRunning(BackendUnavailableError):
    """Raised when the vLLM backend is not running or unreachable."""

    def __init__(self, message: str) -> None:
        super().__init__(message, provider="vllm")


class LocalLLMEndpointNotAvailable(BackendUnavailableError):
    """Raised during startup when the configured local LLM endpoint (ollama/vllm) cannot be reached."""

    def __init__(self, message: str, *, provider: str = "local-llm") -> None:
...
class TesseractPathError(RAGLCCException):
    """Raised when no usable Tesseract executable can be resolved for OCR."""
...
class EmbedModelMismatch(RAGLCCException):
    ...
    Re-run RAGLoad with RETRIEVAL_STORES_KEEP = False to rebuild the collection.
    """
