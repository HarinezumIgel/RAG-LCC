...
class PrettyWriter:
    ...
    def __init__(
        ...
        *,
        ...
        :param always_on: when True, write() ignores the DEBUG_LEVEL == 0 guard
                         and always prints (used for startup / informer output).
        """
    ...
    def terminal_line_size(self) -> int:
        ...
        from Helpers.DebugHelper import \
            DebugHelper  # lazy — avoids circular import

        raw: Any = self.cfg.get("TERMINAL_LINE_SIZE")
        if isinstance(raw, dict):
            key = "debug" if DebugHelper.level(self.cfg) > 0 else "no_debug"
            return int(raw.get(key, raw.get("debug", 160)))  # type: ignore[reportUnknownArgumentType]
    ...
    def write(
        ...
        is_error_or_warning = sev_upper in {"E", "ERROR", "W", "WARNING"}
        if not self.always_on and not is_error_or_warning:
            from Helpers.DebugHelper import \
                DebugHelper  # lazy — avoids circular import

            if DebugHelper.level(self.cfg) == 0:
                return ""
        ...
        # Honour explicit \n in the message by wrapping each segment separately.
        wrapped: list[str] = []
        for segment in stripped_message.split("\n"):
            seg_lines = textwrap.wrap(
                segment,
                width=available,
                break_long_words=False,
                break_on_hyphens=False,
            )
            wrapped.extend(seg_lines if seg_lines else [""])
