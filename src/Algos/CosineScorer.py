...
class CosineScorer(SingletonMixin, ScorerBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.pharase_embedding_cache_tensor: Dict[str, torch.Tensor] = {}
        self.pharase_embedding_cache: Dict[str, List[float]] = {}
    ...
    def build_cosine_cache(self) -> Dict[str, torch.Tensor]:
        ...
        self.pharase_embedding_cache_tensor = self.build_phrase_cache(
            phrases,
            list_cache=self.pharase_embedding_cache,
        )

        self.pretty.write(
            "O",
            "Cosine Scorer",
            f"Built cache for {len(self.pharase_embedding_cache_tensor)} phrase(s).",
        )
        return self.pharase_embedding_cache_tensor
    ...
    def verify(
        ...
        for phrase in self.banned:
            t: torch.Tensor | None = self.pharase_embedding_cache_tensor.get(phrase)
