...
class ClassifyStrategy(SingletonMixin, ProcessingStrategy):
    ...
    def __init__(
        ...
        *,
        ...
        kb_cfg: dict[str, Any] = self.helpers._get_keybert_config()  # type: ignore[reportPrivateUsage]
        ...
        chk_prompt_var: str = self.helpers.get_model_args("_LLM_CHK")["PROMPT_CLASSIFY"]
    ...
    def info(self):
        if self.cfg.get_int("DEBUG_LEVEL") >= 1:
            ...
            if self.cfg.get_int("DEBUG_LEVEL") >= 70:
                line = self.doc.get("content", "N/A").strip()
            elif self.cfg.get_int("DEBUG_LEVEL") >= 1:
