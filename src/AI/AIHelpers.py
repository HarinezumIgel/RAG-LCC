...
from Gui.Colors import GREEN, ORANGE, RED
...
class AIHelpers(SingletonMixin):
    ...
    def check_user_prompt_with_filter_chain(
        ...
        """
        language: str = self.fileUtils.get_text_language(user_prompt, "nltk")

        # --- unsupported-language gate ---
        # For user prompts, NOT_OK falls back to English banlists rather than
        # hard-blocking.  Short or ambiguous queries are frequently
        # misclassified (e.g. "do bees have stingers?" detected as Dutch at
        # 100 % confidence).  Blocking produces false positives that are
        # indistinguishable to the user from genuine compliance rejections.
        # English banlists still run, so the safety check is not skipped.
        ...
        if lang_action == "NOT_OK":
            self.pretty.write(
                ...
                f"falling back to English banlists for user prompt "
                f"(UNSUPPORTED_LANGUAGE_ACTION={lang_action})",
                color=ORANGE,
            )
            language = "english"
    ...
    def check_prompt_with_llm_guard(self, user_query: str) -> Tuple[bool, str]:
        ...
        llm_chk_args = self.helpers.get_model_args("_ACTIVE_LLM_CHK")
        llm_chk_model: str = llm_chk_args["MODEL"]
        chk_prompt_var: str = llm_chk_args["PROMPT_CHAT"]
        prompt_chk_template, prompt_chk_name = self.cfg.indirect_get(chk_prompt_var)
        ...
        compliance_options: dict[str, Any] = {
            ...
            "num_predict": token_budget.compute_dynamic_max_tokens(
                formatted_prompt, llm_chk_model, model_role="_ACTIVE_LLM_CHK"
            ),
