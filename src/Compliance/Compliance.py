...
class Compliance(SingletonMixin):
    ...
    def _fetch_license(self, url: str, label: str, lic_path: str) -> str:
        ...
        msg = f"Fetching license online from: {url}"
        self.logger.warning(msg)
        ...
        try:
            ...
            error_msg = f"Failed to fetch license for [{label}] from URL: {url}. Error: {str(e)}"
    ...
    def _show_pager(self, text: str) -> None:
        ...
        height: int = self._compute_height()
        ...
        while index < len(lines):
            ...
            if index >= len(lines):
                break
            key = (
                input(f"{YELLOW}[Enter] next page, [q] quit view: {RESET}")
                .strip()
                .lower()
            )
            if key == "q":
    ...
    def _check_models_config_hash(self):
        modules = [
            (Config_Models, "_MODELS_CONFIG_HASH"),
            (Config_Banned, "_BANNED_CONFIG_HASH"),
        ]
        mismatches: list[tuple[str, str | None, str, str | None, str]] = []

        for module, cfg_key in modules:
            module_hash = self.fileUtils.hash_module(module)
            ref_hash: str | None = (
                str(self.cfg.get(cfg_key))
                if self.cfg.get(cfg_key) is not None
                else None
            )
            ...
            if module_hash != ref_hash:
                mismatches.append(
                    (module_name, module_path, cfg_key, ref_hash, module_hash)
                )
            else:
                self.pretty.write(
                    "O",
        ...
        if mismatches:
            for module_name, module_path, cfg_key, ref_hash, module_hash in mismatches:
                msg = f"Detected modification of {module_name} ({module_path})"
                self.logger.warning(msg)
                self.pretty.write("W", module_name, msg)

                msg = f"Reference hash {cfg_key}: {ref_hash} "
                self.logger.error(msg)
                ...
                msg = (
                    f"Update Expected hash in Configuration/Config_Global.py "
                    f"{cfg_key} to match expected hash"
                )
                self.logger.error(msg)
    ...
    def _verify_argos_consent(self) -> None:
        ...
        # If packages were just installed and SharedHelpers has no languages
        # yet (first run), refresh so translation works without a restart.
        if installed and not self.sharedHelpers.get_installed_langs():
            self.sharedHelpers.refresh_installed_languages()
